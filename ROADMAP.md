...
| ~~**NetworkPolicies für apps namespace**~~ ✅ | Default-deny + explicit allow in `apps` und `monitoring` deployed (`kubernetes/apps/network-policies.yml`, `kubernetes/system/monitoring/network-policies.yml`). 2026-06-19: Rollout brach zwei Cross-Namespace-Verbindungen stillschweigend (Velero→Garage, Homepage→Uptime Kuma) — beide erst Tage später durch Symptome entdeckt. Bei neuen Cross-Namespace-Abhängigkeiten immer zuerst die NetworkPolicy-Dateien prüfen. |
