...
### REL-031 — Nextcloud/Paperless redis: RDB persistence failing on read-only rootfs, silently blocked all writes · **RESOLVED** (2026-07-05)
...
| IAC-002 | IaC | **CLARIFIED** | Re-checked after REL-038 unblocked the network stack's plan (it had never successfully planned before due to the same backend DNS issue, layered on top of years of never-applied drift). Real plan: destroys the 4 WAN Minecraft port-forward rules (`fwd_wan_minecraft`/`fwd_wan_cobblemon`/`dstnat_minecraft`/`dstnat_cobblemon`) -- **intentional** per PR #216 (2026-07-01, already merged) which moved Minecraft to a playit.gg tunnel instead. Held from applying: the replacement (`mc.woitzik.dev` -> playit.gg CNAME, `cloudflare_dns_record.mc_playit`) is blocked by the same Cloudflare token DNS-scope gap as REL-036's other DNS records (403 on create) -- applying the destroy now would cut external Minecraft access with no working replacement live. Needs the DNS record created (dashboard or broader token) before this is safe to apply (2026-07-04). |
