...
**2026-07-08 update — a load-side change landed ahead of the recheck.** PR #342 added
a CPU limit (`nodeAgent.resources.limits.cpu: 1000m`) to Velero's `node-agent`
DaemonSet — one of the exact candidates listed above, done in response to a live
thermal-load-smoothing request rather than waiting for the 2026-07-10 recheck to
confirm it was needed. This means the 2026-07-10+ recheck now has to be read against
**three simultaneous changes**, not one:

1. REL-062's schedule move to `05:00 UTC`,
2. this CPU limit on `node-agent` (PR #342),
3. the user's own hardware-side rack fan work (outside this repo, timing/details not
   tracked here).

The decision rule above (below-80°C → resolved; still 82-90°C → load-side fix needed)
still applies, but if the recheck shows an improvement, it can no longer be
attributed to the schedule move or cooling work alone — the CPU limit changed the
load shape in the same window. Isolating which of the three actually mattered would
need a controlled test (e.g. temporarily reverting the CPU limit for one night) if
that ever becomes worth doing; not proposing that now, just flagging it so the
2026-07-10+ analysis doesn't silently credit the wrong change.
...
| REL-064 | Reliability | **PENDING, 0/3 nights** | Post-Velero-fix thermal recheck: does peak nightly host temp actually drop below 80C after REL-062's schedule move (05:00 UTC, live as of 2026-07-07 16:03 UTC) plus separate physical cooling work, or does Velero's own IO/CPU load alone keep driving 82-90C regardless of schedule? Can't be answered yet -- the new schedule's first run hasn't happened. Documented pre-fix baseline (7 nights, 81.6-90.0C), exact PromQL methodology, and a decision rule stated in advance (below-80C -> RESOLVED by cooling; still 82-90C -> load-side fix needed, e.g. Velero concurrency limits/ionice/narrower defaultVolumesToFsBackup scope). 2026-07-08: a node-agent CPU limit (PR #342) landed ahead of the recheck -- 2026-07-10+ analysis must be read against 3 simultaneous changes (schedule move, this CPU limit, hardware-side rack fan), not attributed to any one alone. Revisit 2026-07-10 UTC or later (2026-07-07) |
