...
| Question | Doc |
|---|---|
| Where is secret X stored, how does it flow | Ansible Vault (host-level) + HashiCorp Vault via ExternalSecrets (k8s) — see the Stack Overview table in `README.md` |
...
| ExternalSecret stuck on stale "Vault is sealed" error after Vault was unsealed | ClusterSecretStore caches status; force a resync or recreate it | — |
...
- **Secrets rotation**: tracked internally; several credentials found leaked in git
  history early on have since been rotated with live verification (not just moved to
  a different storage location).
- **Velero R2 offsite backup**: still not active — scaffolded but deliberately deferred
  by the account owner, not just "waiting on credentials." See `docs/backup-strategy.md`.
...
- **No native MikroTik config backup**: still true — the Terraform API user doesn't have
  permission to run `/system backup`/`/export` (its live policy explicitly denies
  `password`/`sensitive`). Recoverability depends entirely on Terraform state right now.

## Last known-good audit

This page plus `compute-nodes.md`, `k3s-architecture.md`, `naming-convention.md`,
`backup-strategy.md`, and README get periodically re-verified against live state
directly (`pvesh`, `docker inspect`, `terraform` files, `ansible/site.yml`) — several
have drifted since their last real update in the past. If something here feels stale
again, `git log -1 -- <doc>` tells you how stale.
